...
import banner from '../../assets/events/ml.png';
...
const RiseOfMobaML = () => {
    ...
    return (
        <div className="min-h-screen bg-[#0a0f1a] text-white relative isolate selection:bg-blue-500/30 font-sans">
            ...
            <div className="fixed inset-0 z-[-1]">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-br from-[#0a0f1a]/95 via-[#0a0f1a]/80 to-[#0a0f1a]/90"></div>
            ...
            <div className="pt-32 pb-24 w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="text-center mb-12">
                    <h1 className="text-4xl md:text-6xl font-black italic uppercase font-display mb-2 text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-blue-500 to-indigo-500 drop-shadow-lg leading-tight">
                        Mobile Legends
                        <span className="block text-2xl md:text-4xl">Bang Bang</span>
                    </h1>
                    <p className="text-blue-400/80 font-bold tracking-[0.2em] uppercase text-sm mt-4">Squad Registration</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-8 bg-[#101420]/60 backdrop-blur-md p-6 md:p-10 rounded-3xl border border-blue-500/20 shadow-2xl">

                    {/* Team Details */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-8 border-b border-blue-500/10">
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-blue-400 uppercase tracking-widest">Team Name</label>
                            <input
                                type="text"
                                value={teamName}
                                onChange={(e) => setTeamName(e.target.value)}
                                className="w-full bg-black/40 border border-blue-500/20 rounded-xl px-4 py-3 text-white focus:border-blue-500 focus:outline-none transition-colors"
                                placeholder="Enter Team Name"
                                required
                            />
                        </div>
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-blue-400 uppercase tracking-widest">Country</label>
                            <div className="relative">
                                <select
                                    value={country}
                                    onChange={(e) => setCountry(e.target.value)}
                                    className="w-full bg-black/40 border border-blue-500/20 rounded-xl px-4 py-3 text-white focus:border-blue-500 focus:outline-none transition-colors appearance-none cursor-pointer"
                                >
                                    {countries.map(c => <option key={c} value={c} className="bg-[#101420]">{c}</option>)}
                                </select>
                    ...
                    <div className="space-y-6">
                        <h3 className="text-xl font-black italic uppercase text-white border-l-4 border-blue-500 pl-3">Main Squad</h3>

                        {players.map((player, index) => (
                            <div key={index} className="bg-black/20 p-6 rounded-2xl border border-white/5 hover:border-blue-500/30 transition-colors">
                                <h4 className="text-blue-500/70 font-bold text-xs uppercase tracking-wider mb-4">Player {index + 1} {index === 0 && '(Captain)'}</h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <input
                                        placeholder="Full Name"
                                        value={player.name}
                                        onChange={(e) => handlePlayerChange(index, 'name', e.target.value)}
                                        className="bg-black/30 border border-white/10 rounded-lg px-4 py-3 text-sm focus:border-blue-500/50 outline-none"
                                        required
                                    />
                                    <input
                                    ...
                                    <input
                                        ...
                                        className="bg-black/30 border border-white/10 rounded-lg px-4 py-3 text-sm focus:border-blue-500/50 outline-none md:col-span-2"
                                        required
                                    />
                                </div>
                            </div>
                    ...
                    <div className="space-y-6 pt-4">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="text-xs font-bold uppercase tracking-wider bg-blue-500/10 text-blue-500 px-4 py-2 rounded-lg hover:bg-blue-500/20 transition-colors"
                            >
                                + Add Substitute
                            </button>
                        </div>
                    ...
                    <div className="pt-8">
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl font-black uppercase tracking-[0.2em] text-white hover:from-blue-500 hover:to-indigo-500 transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
                        >
